...
/* Gen b0 */
bool eggp_mutate_edge_b0 = true;

static bool evaluateCondition(void)
{
   return (eggp_mutate_edge_b0);
}

static void evaluatePredicateeggp_mutate_edge_0(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) eggp_mutate_edge_b0 = true;
   else eggp_mutate_edge_b0 = false;
}
   ...
   for(host_index = 0; host_index < eggp_mutate_edge_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateeggp_mutate_edge_0(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               eggp_mutate_edge_b0 = true;
               break;
            }
         }
         else break;
...
void applyeggp_mutate_edge_pickNewTarget(Morphism *morphism, bool record_changes)
{
   ...
   changeNodeMark(eggp_mutate_edge_host, host_node_index, 1);
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < eggp_mutate_edge_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_mutate_edge_pickNewTarget", eggp_mutate_edge_host);
