...
static void evaluatePredicateeggp_init_0(Morphism *morphism)
{
   ...
   if(var_1 > 0) eggp_init_b0 = true;
   else eggp_init_b0 = false;
}

static bool match_n0(Morphism *morphism);
static bool match_n1(Morphism *morphism);

bool matcheggp_init_addNode(Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(eggp_init_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateeggp_init_0(morphism);
...
void applyeggp_init_addNode(Morphism *morphism, bool record_changes)
{
   string var_0 = getStringValue(morphism, 0);
   int var_1 = getIntegerValue(morphism, 1);
   int var_2 = getIntegerValue(morphism, 2);
   int host_node_index = lookupNode(morphism, 0);
   HostLabel label_n0 = getNodeLabel(eggp_init_host, host_node_index);
   HostLabel label;
   ...
   else
   {
      ...
      relabelNode(eggp_init_host, host_node_index, label);
   }
   host_node_index = lookupNode(morphism, 1);
   HostLabel label_n1 = getNodeLabel(eggp_init_host, host_node_index);
   ...
   int node_array_size2 = eggp_init_host->nodes.size;
   ...
   host_node_index = addNode(eggp_init_host, 0, label);
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size2 == eggp_init_host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, eggp_init_host);
}
...
bool fillpoteggp_init_addNode(MorphismPot *pot, Morphism *morphism)
{
   if(2 > eggp_init_host->number_of_nodes || 0 > eggp_init_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(eggp_init_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_init_addNode", eggp_init_host);
